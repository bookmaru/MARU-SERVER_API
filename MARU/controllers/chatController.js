...
const chat = {
  ...
  getUnread: async (req, res) => {
    ...
    if (!roomIdx1){ roomIdx1=0; }
    if (!roomIdx2){ roomIdx2=0; }
    if (!roomIdx3){ roomIdx3=0; }
    ...
    try {
      ...
      res.status(statusCode.OK).send(util.success(statusCode.OK, resMessage.SUCCESS_GET_UNREAD, 
        {getCount:getCount, getAlready: getAlready, getUnread: getUnread}));
      return;
    } catch (err) {
      res.status(statusCode.DB_ERROR).send(util.fail(statusCode.DB_ERROR, resMessage.SERVER_ERROR));
    }
